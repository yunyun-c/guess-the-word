...
const updateGuessesRemaining = function (guess) {
    ...
    if (!upperWord.includes(guess)) {
        ...
        message.innerText = `good guess! The word has th letter ${guess}.`;
    }

    if (remainingGuesses === 0) {
        message.innerHTML = `Game over! Then word was <span class="highlight">${word}</span>.`;
    } else if (remainingGuesses === 1) {
        numOfremainig.innerText = `${remainingGuesses} guess`;
    }else {
        numOfremainig.innerText = `${remainingGuesses} guesses`;
    }
};
